    ...
    <SiteFrame locale={locale} copy={copy}>
      <section className="relative min-h-screen px-4 pb-24 pt-32 sm:px-6">
        ...
        <div className="mx-auto max-w-5xl">
          <Link href={`/${locale}`} className="mb-10 inline-flex items-center gap-2 text-sm font-black uppercase text-bone/60 transition hover:text-flare">
            <ArrowLeft size={16} />
            {copy.nav.home}
          </Link>
          <div className="rounded-[8px] border border-white/10 bg-carbon/70 p-6 shadow-2xl backdrop-blur sm:p-10">
            <div className="mb-5 inline-flex items-center gap-2 rounded-[8px] border border-ember/50 bg-ember/10 px-3 py-2 text-sm font-black uppercase text-flare">
              <Zap size={16} />
              {content.eyebrow}
            </div>
            <h1 className="max-w-4xl text-5xl font-black leading-tight text-bone sm:text-7xl">{content.title}</h1>
